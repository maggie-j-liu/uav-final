...
class TelloOpticalFlow:
    def __init__(self):
        ...
        # helps decide what corners should be tracked
    ...
    def sparse_optical_flow_lk(self, led_mask):
        ...
        # add mask from led detection so that we have a ROI
        corners0 = cv2.goodFeaturesToTrack(
            frame0_gray, led_mask, **self.feature_params)
        ...
        while True:
            ...
            if found_current.shape == found_prev.shape:
                ...
                avg_change = np.sum(changes, axis=0) / \
                    changes.shape[0]  # assuming ROI
